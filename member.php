...
if($cfg_oauth == 'Y')
{
	require_once(XIANYU_DATA.'/api/oauth/system/core.php');
}
   ...
   !empty($_COOKIE['lastloginip']))
{
	...
	$datavalue     = AuthCode($_COOKIE['datavalue']);
	$dataname     = AuthCode($_COOKIE['dataname']);
	$live_city     = AuthCode($_COOKIE['live_city']);
	$live_prov     = AuthCode($_COOKIE['live_prov']);
	$live_country     = AuthCode($_COOKIE['live_country']);
...
if($a == 'login')
{
		...
		if(check_app_login('qq') or check_app_login('weibo'))
		{
			...
			else
			{
				...
				else
				{
					...
					setcookie('user_id',      AuthCode($row['id'] ,'ENCODE'), $cookie_time);
					setcookie('live_city',      AuthCode($row['live_city'] ,'ENCODE'), $cookie_time);
					setcookie('live_prov',      AuthCode($row['live_prov'] ,'ENCODE'), $cookie_time);
					setcookie('live_country',      AuthCode($row['live_country'] ,'ENCODE'), $cookie_time);
	...
	else
	{
		...
		if(is_array($r))
		{
			...
			$row = $dosql->GetOne("SELECT `id`,`password`,`logintime`,`loginip`,live_city,`live_prov`,live_country,is_list,`expval` FROM `#@__member` WHERE `username`='$username'");
			...
			else
			{
				...
				//是否自动登录
				if(isset($autologin))
				$cookie_time = time()+(20*360)*24*60*60;
				
				setcookie('username',      AuthCode($username        ,'ENCODE'), $cookie_time);
				setcookie('datavalue',      AuthCode(1        ,'ENCODE'), $cookie_time);
				setcookie('dataname',      AuthCode('阳光'        ,'ENCODE'), $cookie_time);
				setcookie('user_id',      AuthCode($row['id']       ,'ENCODE'), $cookie_time);
				setcookie('live_city',      AuthCode($row['live_city']       ,'ENCODE'), $cookie_time);
				setcookie('live_prov',      AuthCode($row['live_prov']       ,'ENCODE'), $cookie_time);
				setcookie('live_country',      AuthCode($row['live_country']       ,'ENCODE'), $cookie_time);
				setcookie('is_list',      AuthCode($row['is_list']       ,'ENCODE'), $cookie_time);
				setcookie('lastlogintime', AuthCode($row['logintime'],'ENCODE'), $cookie_time);
				setcookie('lastloginip',   AuthCode($row['loginip']  ,'ENCODE'), $cookie_time);
...
else if($a == 'logout')
{
	...
	setcookie('live_prov',     '', time()-3600);
	setcookie('live_city',      '', time()-3600);
	setcookie('live_country',      '', time()-3600);
	setcookie('is_list',      '', time()-3600);
...
//支付余额
else if($a == 'set_insurance')
{
	$cookie_time = time()+(20*360)*24*60*60;
	$r = $dosql->GetOne("SELECT `dataname` FROM `#@__cascadedata` WHERE `datagroup`='insurance' and `datavalue`='{$datavalues}'");
	if($r){
		setcookie('datavalue',      AuthCode($datavalues        ,'ENCODE'), $cookie_time);
		setcookie('dataname',      AuthCode($r['dataname']       ,'ENCODE'), $cookie_time);
		echo '切换成功,请刷新';
	}else{
		echo '切换失败,请刷新重新尝试';
...
{
	$id=(int)$id;
	$r = $dosql->GetOne("SELECT `id` FROM `#@__infoimg` WHERE `id`='$id'");
	if(empty($r) && !is_array($r))
	{
		echo '案件不存在';
		exit;
	...
	{
		$appointment = GetMkTime($appointment);
		$a =  $dosql->ExecNoneQuery("UPDATE `#@__infoimg` SET `appointment`='$appointment' WHERE `id`='$id'");
		if($a){
...
{
	$id=(int)$id;
	$r = $dosql->GetOne("SELECT `id`,live_prov,live_city,live_country FROM `#@__member` WHERE `username`='$c_uname'");
	if(empty($r) && !is_array($r))
	{
	...
	else
	{
		...
		$dosql->Execute("SELECT id,cnname FROM `#@__member` WHERE id!=$user_id and  live_prov='{$r['live_prov']}' and live_city='{$r['live_city']}' and (is_list=".$isold_part.' or is_list=2)');
		while($row = $dosql->GetArray())
		{
			if($row['username']!=$c_uname){
				$op.='<option value='.$row['id'].'>'.$row['cnname'].'</option>';
			}
		}
	}
   echo $op;
...
}else if($a == 'upusers')
{
	...
	else
	{
		...
		$dosql->Execute("SELECT id,cnname FROM `#@__member` WHERE id!=$user_id and  live_prov='{$r['live_prov']}' and live_city='{$r['live_city']}' and (is_list=1 or is_list=2)");
		while($row = $dosql->GetArray())
		{
			if($row['username']!=$c_uname){
				$op.='<option value='.$row['id'].'>'.$row['cnname'].'</option>';
   ...
   echo $op;
	exit();
}else if($a == 'choiceuser')
{
	$uid=(int)$uid;
	$id=(int)$id;
	$a =  $dosql->ExecNoneQuery("UPDATE `#@__infoimg` SET `user_id`='$uid' WHERE `id`='$id'");
	echo '已转交成功';
	exit();
}
else if($a == 'complete')
{	
	$picarr='';
	if($staff_picarrs!=''){
	$staff_picarrsarr=array();
	$pieces_arr = explode(",", $staff_picarrs);
	
	foreach($pieces_arr as $_v){
		if($_v!=''){
			$staff_picarrsarr[]=$_v;
		}
	}
	if(is_array($staff_picarrsarr))
	{
		$picarr = serialize($staff_picarrsarr);
	}
	}
	$remarks = htmlspecialchars($remarks);
	
	$id=(int)$id;
	$a =  $dosql->ExecNoneQuery("UPDATE `#@__infoimg` SET `staff_picarrs`='$picarr',`istate`='1',remarks = '$remarks' WHERE `id`='$id'");
	echo '提交成功';
	exit();
}else if($a == 'oldcasecomplete')
{	
	$picarr='';
	if($staff_picarrs!=''){
	$staff_picarrsarr=array();
	...
	$a =  $dosql->ExecNoneQuery("UPDATE `#@__infoimg` SET `staff_picarrs`='$picarr',`ostate`='1',remarks = '$remarks',address='$address' WHERE `id`='$id'");
	echo '提交成功';
	exit();
}else if($a == 'oldcasecompletes')
{	
	$picarr='';
	if($staff_picarrs!=''){
	$staff_picarrsarr=array();
	...
	$id=(int)$id;
	$a =  $dosql->ExecNoneQuery("UPDATE `#@__infoimg` SET `ostaff_picarrs`='$picarr',`ostate`='1',oremarks = '$remarks' WHERE `id`='$id'");
	echo '提交成功';
	exit();
}
else if($a == 'oldcase')
{
	...
	$address = htmlspecialchars($address);
	$id=(int)$id;
	$a =  $dosql->ExecNoneQuery("UPDATE `#@__infoimg` SET `ouid`='$useid', isold_part=1,`staff_picarrs`='$picarr',remarks='$remarks',`istate`='1',address='$address'  WHERE `id`='$id'");
	echo '提交成功';
	exit();
}else if($a == 'setaddress')
{
	$cookie_time = time()+3600;
	setcookie('relocationaddress', AuthCode($address,'ENCODE'), $address);
...
if($c == 'login')
{
	...
	{
		require_once(XIANYU_TEMP.'/default/member/login.php');
		exit();
	}
}
...
{
	if($c_uname != 'guest')
		require_once(XIANYU_TEMP.'/default/member/default.php');
	else
		require_once(XIANYU_TEMP.'/default/member/defaultguest.php');
...
{
	require_once(XIANYU_TEMP.'/default/member/avatar.php');
	exit();
}
...
{
	require_once(XIANYU_TEMP.'/default/member/edit.php');
	exit();
}
...
{
	require_once(XIANYU_TEMP.'/default/member/comment.php');
	exit();
}
...
{
	require_once(XIANYU_TEMP.'/default/member/favorite.php');
	exit();
}
...
{
	require_once(XIANYU_TEMP.'/default/member/order.php');
	exit();
}
...
{
	require_once(XIANYU_TEMP.'/default/member/ordershow.php');
	exit();
}
...
{
	require_once(XIANYU_TEMP.'/default/member/msg.php');
	exit();
}
...
else if($c == 'perfect')
{
	if(isset($c_uname) && $c_uname == 'guest')
		require_once(XIANYU_TEMP.'/default/member/perfect.php');
...
{
	if(isset($c_uname) && $c_uname == 'guest')
		require_once(XIANYU_TEMP.'/default/member/binding.php');
	else if(isset($c_uname) && $c_uname != 'guest')
		header('location:?c=default');
	else
		header('location:?c=login');
...
{
	require_once(XIANYU_TEMP.'/default/member/reg.php');
	exit();
}
...
{
	require_once(XIANYU_TEMP.'/default/member/findpwd.php');
	exit();
}
...
else if($c == 'findpwd2')
{
	if(!isset($_POST['username']))
		...
		require_once(XIANYU_TEMP.'/default/member/findpwd2.php');
...
else if($c == 'findpwd3')
{
	if(!isset($_POST['uname']))
		...
		require_once(XIANYU_TEMP.'/default/member/findpwd3.php');

	exit();
}

else if($c == 'investigation')
{	if(!$c_uname){
		header('location:?c=login');
		exit;
		
	}
	require_once(XIANYU_TEMP.'/default/member/investigation.php');
	exit();
}
else if($c == 'case')
{	if(!$c_uname){
	...
	require_once(XIANYU_TEMP.'/default/member/case.php');
	exit();
}
else if($c == 'oldparts')
{	if(!$c_uname){
	...
	require_once(XIANYU_TEMP.'/default/member/oldparts.php');
	exit();
}
else if($c == 'communication')
{	if(!$c_uname){
		header('location:?c=login');
		exit;
		
	}
	require_once(XIANYU_TEMP.'/default/member/communication.php');
	exit();
}
else if($c == 'ajaxgetinvestigationlist')
{
	require_once(XIANYU_TEMP.'/default/member/ajaxgetinvestigationlist.php');
	exit();
}else if($c == 'casecontent')
{
	if(!$c_uname){
		header('location:?c=login');
		exit;
	...
	require_once(XIANYU_TEMP.'/default/member/casecontent.php');
	exit();
}else if($c == 'casecontents')
{
	if(!$c_uname){
		header('location:?c=login');
		exit;
	...
	require_once(XIANYU_TEMP.'/default/member/casecontents.php');
	exit();
}else if($c == 'img')
{
	if(!$c_uname){
	...
	require_once(XIANYU_TEMP.'/default/member/img.php');
	exit();
}else if($c == 'imgs')
{
	if(!$c_uname){
	...
	require_once(XIANYU_TEMP.'/default/member/imgs.php');
	exit();
}else if($c == 'ajaxcommunicationlist')
{
	require_once(XIANYU_TEMP.'/default/member/ajaxcommunicationlist.php');
	exit();
}
